...
- `Up/Down/Left/Right` or `k/j/h/l` - Navigate
- `Enter` - Confirm
...
- `Up/Down` or `k/j` - Navigate up/down
- `/` - Enter search mode
...
2. Creates `/etc/locale.conf` with all LC\_\* variables
...
{
  "minimal": {
    "pacman": [{ "package": "my-de-core" }, { "package": "display-manager" }],
    "aur": []
  },
  "full": {
    "pacman": [{ "package": "my-de-full" }],
    "aur": [{ "package": "my-de-themes" }]
  }
}
```
