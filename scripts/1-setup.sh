...
if [[ ! $DESKTOP_ENV == server ]]; then
  sed -n '/'$INSTALL_TYPE'/q;p' $HOME/ArchInstaller/pkg-files/pacman-pkgs.txt | while read line
  do
...
echo "username=${username,,}" >> ${HOME}/ArchInstaller/configs/setup.conf

    #Set Password
    read -p "Please enter password:" password
echo "password=${password,,}" >> ${HOME}/ArchInstaller/configs/setup.conf
    ...
    echo "NAME_OF_MACHINE=${name_of_machine,,}" >> ${HOME}/ArchInstaller/configs/setup.conf
fi
echo -ne "
-------------------------------------------------------------------------
                    Adding User
